...
DF$re <- c(apply(myList$z_jlt[,2,],2,function(x){return(x[knots_xy$nn.idx])}))
...
DF$mu <- opt$par['mu']
# DF$yr <- yr_ef$ef.1[match(DF$tt,yr_ef$y)]
# DF$j <- j_ef$ef.1[match(round(DF$x),j_ef$j)]
# DF$l <- l_ef$ef.1[match(round(DF$y),l_ef$l)]
DF$Survival <- plogis(DF$mu+DF$R.E.)

DF2 <- data.frame(x=data$j_i+minJ, y=data$l_i+minL, tt=data$t_i+1998, Observed=data$surv/data$total)

sampleYears <- c(1998:2019)#c(1998:2000,2017:2019)
breaks <- c(-1,0,1)
