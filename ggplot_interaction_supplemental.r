...
breaks <- c(-1,0,1)
p1 <- ggplot(data = DF[,], aes(x = as.Date(x, origin = as.Date("2018-01-01")), y = y, 
                               fill = R.E.)) +
                               geom_raster()+
  facet_wrap(~tt, ncol=4) +
  ylab('Length (mm)') +
  xlab('') +
...
# tiff("ggplot_interaction_supplemental.tiff", height=820, width=820)
png("ggplot_interaction_supplemental.png", height=820, width=820, res=100)
p <- ggarrange(p1, 
          ncol = 1)
