f_range_sim <- function(fit = fit,
  ...
  x0.5 <- round(sum(na.omit(c(proj_sim[,1,1:2,action,])<0))/length(na.omit(c(proj_sim[,1,1:2,action,])))*100,1)
  df <- data.frame(val = c(proj_sim[,1,1:2,action,]),
                   var=rep(paste0("0.5x (",x0.5,"%)"),length(c(proj_sim[,1,1:2,action,]))))
  
  x1 <- round(sum(na.omit(c(proj_sim[,2,1:2,action,])<0))/length(na.omit(c(proj_sim[,2,1:2,action,])))*100,1)
  df <- rbind(df,
              data.frame(val = c(proj_sim[,2,1:2,action,]),
                         var=rep(paste0("1x (",x1,"%)"),length(c(proj_sim[,2,1:2,action,])))))
  
  x2 <- round(sum(na.omit(c(proj_sim[,3,1:2,action,])<0))/length(na.omit(c(proj_sim[,3,1:2,action,])))*100,1)
  df <- rbind(df,
              data.frame(val = c(proj_sim[,3,1:2,action,]),
                         var=rep(paste0("2x (",x2,"%)"),length(c(proj_sim[,3,1:2,action,])))))
